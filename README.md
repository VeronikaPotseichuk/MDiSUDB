...
* Логирование
2. **Работа с сайтом**: 
- Администратор создаёт профили новых сотрудников (ФИ, адрес, пол, дату рождения, телефон, логин, пароль, позицию и заработную плату).
- Администратор следит за инвентарём и закупками.
- Администратор следит за финансами мастерской (доходы и расходы).
- Менеджер вносит данные клиентов (ФИ, адрес, телефон).
- Менеджер принимает заказы клиентов и берёт предоплату/полную оплату за оказанную услугу.
- Менеджер оказывает услугу продажи готового изделия и создаёт счёт (и помечает способ оплаты).
- Мастер-ювелир создаёт заказ и описывает количество потраченного материала.
- Менеджер создаёт счёт на основе описания мастера-ювелира.
- Менеджер рассчитывает клиента и помечает способ оплаты.
